...
public class IntersectionPointOfLL {
  ...
  public static class LinkedList {
    ...
    public static int findIntersection(LinkedList one, LinkedList two){
      int skipTill = Math.abs(one.size()-two.size());
    
      Node node1=one.head;
      Node node2=two.head;

      for(int index=0;index<skipTill;index++){
      ...
      int runTill = one.size()>two.size()?one.size()-skipTill:two.size()-skipTill;
      
      for(int start=0;start<runTill;start++){
