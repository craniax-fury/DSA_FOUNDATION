...
public class KLevelsDown {
    ...
    public static void printKLevelsDown(BinaryNode node, int k) {
        Stack<LevelPair> stack = new Stack<>();

        stack.push(new LevelPair(node, 0));

        while (!stack.isEmpty()) {

            if (stack.peek().level == k) {
                System.out.println(stack.peek().node.data);
            }

            LevelPair tempNodeLevelPair = stack.pop();
            if (tempNodeLevelPair.level != k) {
                if (tempNodeLevelPair.node.right != null) {
                    stack.push(new LevelPair(tempNodeLevelPair.node.right, tempNodeLevelPair.level + 1));
                }

                if (tempNodeLevelPair.node.left != null) {
                    stack.push(new LevelPair(tempNodeLevelPair.node.left, tempNodeLevelPair.level + 1));
                }
            }
        }
